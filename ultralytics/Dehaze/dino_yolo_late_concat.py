...
class _DinoLateConcatHook(nn.Module):
    def __init__(
        ...
        reduce_ratio: int,
        alpha_init: float,
        use_levels: tuple[bool, bool, bool],
        diag: str,
        diag_batches: int,
    ):
        super().__init__()
    ...
    def detect_pre_hook(self, module, inputs):
        ...
        mean = _IMAGENET_MEAN.to(imgs.device)
        std = _IMAGENET_STD.to(imgs.device)
        ...
        if self.proj is None:
            ...
            for li in range(3):
                ...
                self.head.append(
                    nn.Sequential(
                        nn.Conv2d(cin_y + red, cin_y + red, kernel_size=3, padding=1, groups=cin_y + red, bias=False),
        ...
        for li in range(3):
            ...
            if not self.use_levels[li]:
                fused.append(y)
                deltas.append(torch.tensor(0.0, device=y.device))
                continue
        ...
        if self.diag != "none" and self.batches_seen < self.diag_batches:
            if self.diag == "zero_p3":
                ...
                fused[0] = F.interpolate(
                    self.proj[0](dino_feats[0]), size=feats[0].shape[-2:], mode="bilinear", align_corners=False
                )
