...
class DinoConvNeXt(nn.Module):
    def __init__(self, name="convnext_small.dinov3_lvd1689m", pretrained=True, freeze=True, out_indices=(1, 2, 3)):
        super().__init__()
        import timm
...
class ConcatReduce(nn.Module):
    """out = yolo + sigmoid(alpha) * mixed - adapt: 1x1 把 DINO 通道压到更小 - mix: (yolo ⊕ adapt(dino)) → 3x3 + BN + SiLU +
    3x3 + BN - 初始 BN(weight)=0, bias=0，alpha=0 => 初值完全等价纯 YOLO.
    """
...
class YOLO12WithDINO(nn.Module):
    def __init__(
        ...
        self.fuse = nn.ModuleList([ConcatReduce(yc, dc, shrink_ratio=reduce_ratio) for yc, dc in zip(yolo_ch, dino_ch)])
