...
def parse_args():
    ...
    ap.add_argument("--splits", type=float, nargs=3, default=[0.7, 0.2, 0.1],
    ...
    ap.add_argument("--names", type=str, nargs="*", default=None,
                    help="Explicit class names order (optional)")
    ap.add_argument("--ignore-imagesets", action="store_true",
...
def robust_split(items: List[Path], ratios: Tuple[float,float,float]) -> dict:
    ...
    if n >= 3:
        ...
        if n_test  == 0: n_test,  n_val  = 1, n_val  - 1
    return {
        "train": items[:n_train],
...
def main():
    ...
    if not ann_dir.exists() or not img_dir.exists():
        print(f"[ERR] Not found: {ann_dir} or {img_dir}", file=sys.stderr); sys.exit(1)

    xmls = sorted(ann_dir.glob("*.xml"))
    if not xmls:
        print(f"[ERR] No XML files under {ann_dir}", file=sys.stderr); sys.exit(1)

    # 类别统计/顺序
    cls_counter = Counter()
    for xp in xmls:
        _, _, boxes = parse_voc_xml(xp)
        for c, *_ in boxes: cls_counter[c] += 1
    ...
    print(f"[INFO] Random split -> train={len(split_xmls['train'])}, val={len(split_xmls['val'])}, test={len(split_xmls['test'])}")
