...
def dark_channel(im, r=7):
    """im: float32, range[0,1], shape HxWx3 r: window radius (patch size = 2r+1).
    """
    # 像素级最小通道
...
def estimate_atmospheric_light(im, dark, top_percent=0.001):
    """从暗通道中取 top 百分比(值大的位置)，在原图上选强度最大的像素作为 A（逐通道取RGB） im: float32 [0,1], HxWx3 dark: float32 [0,1], HxW.
    """
    H, W = dark.shape
...
def guided_filter(I, p, r=40, eps=1e-3):
    """引导滤波（灰度版）：I 为导向图(灰度, [0,1]), p 为待滤波图(单通道, [0,1]) 返回 q 参考: K. He, J. Sun, X. Tang, Guided Image Filtering.
    """
...
def process_dir(indir, outdir, exts=(".jpg", ".jpeg", ".png"), keep_rel=True, **kwargs):
    ...
    for p in indir.rglob("*"):
        if p.suffix.lower() in exts:
            ...
            dst = outdir / rel
            ok, _msg = process_one(p, dst, **kwargs)
            n_ok += int(ok)
    return n_ok, n_all
...
def main():
    ...
    ap.add_argument(
        "--top_percent", type=float, default=0.001, help="top percentage for A estimation (e.g., 0.001 = 0.1%)"
    )
    args = ap.parse_args()
